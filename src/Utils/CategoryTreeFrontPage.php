...
class CategoryTreeFrontPage extends CategoryTreeAbstract
{
    ...
    public function getCategoryList(array $categoriesArray): string
    {
        ...
        foreach ($categoriesArray as $value) {
            ...
            $catName = $value['name'];
            $catId = $value['id'];
            $url = $this->urlGenerator->generate('video_list', ['categoryName' => $this->slugger->slugify($catName), 'id' => $catId]);

            $this->categoryList .= $this->html_3 . $url . $this->html_4 . $catName . $this->html_5;
    ...
    public function getMainParent(int $id): array
    {
        ...
        if ($this->categoriesArrayFromDb[$key]['parent_id'] !== null) {
            ...
            return ['id' => $this->categoriesArrayFromDb[$key]['id'], 'name' => $this->categoriesArrayFromDb[$key]['name']];
        }
    }
