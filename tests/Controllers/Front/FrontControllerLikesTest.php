...
class FrontControllerLikesTest extends WebTestCase
{
    ...
    public function testNumberOfLikedVideosLikeTest()
    {
        ...
        $this->assertSame(2, $crawler->filter('table tbody tr')->count());

        $this->client->request('POST', '/video-list/11/like');
        $this->client->request('POST', '/video-list/11/like');

        $crawler = $this->client->request('GET', '/admin/videos');
        $this->assertSame(3, $crawler->filter('table tbody tr')->count());
    }

    public function testNumberOfLikedVideosUnlikeTest()
    {
        $crawler = $this->client->request('GET', '/admin/videos');
        $this->assertSame(2, $crawler->filter('table tbody tr')->count());
        ...
        $crawler = $this->client->request('GET', '/admin/videos');
        $this->assertSame(0, $crawler->filter('table tbody tr')->count());
    }
}
